export
const PROTO_PATH = __dirname + '/Chk.proto';//path of the proto file

const grpc = require('grpc');
const protoLoader = require('@grpc/proto-loader');
const packageDefinition = protoLoader.loadSync(
  PROTO_PATH,{});
const app = grpc.loadPackageDefinition(packageDefinition);
const chkpackage = app.chkpackage;

function main(){ 
var client = new chkpackage.ChineseCheckers('https://xyz-fa4fc7hwwq-uc.a.run.app',
        grpc.credentials.createInsecure());
        
        

var Token="eyJhbGciOiJSUzI1NiIsImtpZCI6IjM1MDM0MmIwMjU1MDAyYWI3NWUwNTM0YzU4MmVjYzY2Y2YwZTE3ZDIiLCJ0eXAiOiJKV1QifQ..FSt3S1UTT65ozZEcLQIDiFidSLjsAJQdaToDqxN2SzC1AuKokYA5RC1FxXibJwh31p7kkB3FEwuZGVOFQcVDqqqdr8sBUeCWqmXKR_4PORDM30HpC0HxSf36EtgiiZ662USl3fogJ77KMGDKljl3l4C-Hs6_rmF_86ivkqfflhldmtmPYLgz9BzejuTzgdxEA35wop4_AA6Bd5otJVOuEoXPEAArmrsJGx1scfHI1zeY_2AjEgLlqjKFdn1qQIMTU7OgxdKPbnyeNX8H2fFUsRfwn9GxJNEIg01_fVaWTg63EYdwr7X7GF7QaRHOCy2sXxQ8lraZ0tnaAI_0H0XnVg"


client.createRoomCode( Token ,(err:any,response:object)=> {
  console.log('Roomcode:'+ JSON.stringify(response));})
}
main();